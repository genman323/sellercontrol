...
local function moveTo(pos, offsetY)
    ...
    connections.setup = RunService.Heartbeat:Connect(function()
        if currentSetup == 'setup' and root then
            root.CFrame = targetCFrame
            root.Velocity = Vector3.zero
...
local locations = {
    club       = function() resetState() moveTo(Vector3.new(-264.9, -6.2, -374.9), 2.8) end,
    bank       = function() resetState() moveTo(Vector3.new(-375, 16, -286), 2.8) end,
    boxingclub = function() resetState() moveTo(Vector3.new(-263, 53 - 2.8, -1129), 2.8) end,
    basketball = function() resetState() moveTo(Vector3.new(-932, 21 - 5 + 0.3 + 0.6, -483), 2.8) end,
    soccer     = function() resetState() moveTo(Vector3.new(-749, 22 - 5 + 1.2, -485), 2.8) end,
    cell       = function() resetState() moveTo(Vector3.new(-295, 21 - 3, -111), 2.8) end,
    cell2      = function() resetState() moveTo(Vector3.new(-295, 22 - 3, -68), 2.8) end,
    school     = function() resetState() moveTo(Vector3.new(-654, 21 - 3, 256), 2.8) end,
    train      = function() resetState() moveTo(Vector3.new(636, 47 - 5, -80), 2.8) end,
}
